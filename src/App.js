...
class App extends Component {
  ...
  filterResults() {
    let filtered = this.state.masters.filter(master => {
      ...
      return masterLowerCase.includes(this.state.filter)
    })
    return filtered;
  }
      ...
      <div className="App">
        < Nav search={this.handleSearchInput}/>
          <Switch>
            ...
            <Route exact path="/masters" render={(props) => <MastersContainer masters={results} />} />
            ...
            <Route exact path="/masters/:name" render={(renderProps) => {
                ...
                let previousMaster = foundMasterIndex > 0 ?  this.state.masters[foundMasterIndex-1] : null

                // whats your index
                /// therefore whats the next index
                /// who is the next index
                return <Masters master={foundMaster} nextMaster={nextMaster} prevMaster={previousMaster}/>
                } } />
          </Switch>
      </div>
    );
